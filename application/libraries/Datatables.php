  ...
  class Datatables
  {
    ...
    */
    public function like($key_condition, $val = NULL, $side = 'both')
    {
      ...
      $this->ci->db->ilike($key_condition, $val, $side);
      return $this;
    }
    /**
    ...
    */
    public function or_like($key_condition, $val = NULL, $side = 'both')
    {
      ...
      $this->ci->db->or_ilike($key_condition, $val, $side);
      return $this;
    }
    /**
      ...
      if($sSearch != '')
        for($i = 0; $i < count($mColArray); $i++)
          if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns))
            if($this->check_cType())
              $sWhere .="cast(".$this->select[$mColArray[$i]['data']] . " as text) iLIKE '%" . $sSearch . "%' OR ";
            else
              $sWhere .="cast(".$this->select[$this->columns[$i]] . " as text) iLIKE '%" . $sSearch . "%' OR ";
    ...
    */
    private function get_total_results($filtering = FALSE)
    {
      ...
      foreach($this->like as $val)
        $this->ci->db->ilike($val[0], $val[1], $val[2]);
      foreach($this->or_like as $val)
        $this->ci->db->or_ilike($val[0], $val[1], $val[2]);
